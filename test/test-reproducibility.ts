...
async function benchmark<T>(cb: () => Promise<T>): Promise<{ duration: number, result: T }> {
...
async function testTtfbAndRate(t: ExecutionContext<MyProperties>, ttfb: number, rate: number, size: number) {
  ...
  t.true(Math.abs(withoutProxy.duration - expected) < expected * 0.1, 'Without proxy')
  ...
  t.true(Math.abs(withOfflineProxy.duration - expected) < expected * 0.1, 'With offline proxy')
}
